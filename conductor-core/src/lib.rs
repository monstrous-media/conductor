...
//! - **conductor-daemon**: Background service with config hot-reload
//! - **conductor**: Legacy direct-run application (deprecated)
...
pub mod dispatch; // Action dispatch result types (ADR-009 Gap 1)
pub mod engine;
pub mod error;
pub mod event_processor;
pub mod event_types; // Type-safe event/pattern enums (ADR-004)
pub mod events;
pub mod execution_context; // Per-event evaluation context (ADR-025)
pub mod feedback;
pub mod gamepad_events; // Gamepad/HID input mapping
pub mod gamepad_filters; // Gamepad stream-quality filters
pub mod identity; // Device identity types (ADR-009)
pub mod mapping; // Public for advanced event processing
pub mod midi_output; // MIDI output management
pub mod osc_pattern; // OSC 1.0 address pattern matching (ADR-039-A)
pub mod resolver; // Port resolver for multi-device binding (ADR-009)
pub mod rule_compiler; // Config → CompiledRuleSet compiler (ADR-009 Phase 3)
pub mod rule_set; // Lock-free compiled rule set (ADR-009 Phase 3)
pub mod transform;
pub mod velocity; // Velocity mapping calculations // MIDI transform pipeline (ADR-009 Gap 2)
...
// ADR-039 cross-protocol substrate
...
// Plugin ID validation (ADR-027 D10c). Single source of truth for the
// plugin-id character
