...
use conductor::{
    config::{ActionConfig, Config, Mapping, Mode, Trigger},
...
#[test]
fn global_mapping_fires_when_mode_specific_has_no_match() {
    let mut engine = MappingEngine::new();
    let config = Config {
        mcp: Default::default(),
        config_meta: Default::default(),
        ...
        // A global mapping for a DIFFERENT note than the mode handles.
        global_mappings: vec![Mapping {
            trigger: Trigger::Note {
                note: 99,
                velocity_min: None,
    ...
    // Note 99 matches the GLOBAL mapping but no mapping in (existing) mode 0.
    let event = MidiEvent::NoteOn {
        note: 99,
        velocity: 100,
        channel: 0,
    ...
    let mode_event = MidiEvent::NoteOn {
        note: 60,
        velocity: 100,
        channel: 0,
        time: Instant::now(),
    };
    assert!(engine.get_action(&mode_event, 0).is_some());
}
