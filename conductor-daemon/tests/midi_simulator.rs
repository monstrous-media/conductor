...
/// root crate's copy, which lacks this const) fails to compile.
///
/// `allow(dead_code)`: only referenced from the bin's `#[cfg(test)]` assertion,
/// so it is unused in the bin's normal build and when this file compiles as a
/// standalone test binary.
#[allow(dead_code)]
pub const HELPER_CRATE: &str = "conductor-daemon";
...
impl MidiSimulator {
    ...
    /// (which drains), this is the read-only accessor for repeated inspection.
...
mod tests {
    ...
    fn test_velocity_ramp_single_step_uses_min() {
        ...
        let events = sim.get_events();
        assert_eq!(events.len(), 2); // one note-on + one note-off
