...
//! ADR-027 D1 integration test for [`classify_peer`].
//!
//! The prior stage pinned the kernel-handle for liveness; this stage maps a
...
/// On the happy path the test cleans up explicitly and calls
...
fn classify_peer_returns_untrusted_for_foreign_binary() {
    ...
    // the downstream GUI binary). On macOS its code-signature is either
    ...
    // below would otherwise leak it on panic. Disarmed after the
    ...
    // — the macOS path inspects the peer via Security.framework
