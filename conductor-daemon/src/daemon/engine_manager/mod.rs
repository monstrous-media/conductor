...
// ── Submodules: decompose engine_manager ────────────────
...
/// Maximum events stored in event monitor buffer
...
/// 3. `current_mode` (ArcSwap) - Mode selection, LOCK-FREE (ADR-009 Phase 3)
/// 4. `event_processor` (RwLock) - Event processing, uses config state
/// 5. `rule_set` (ArcSwap) - Rule matching, LOCK-FREE (ADR-009 Phase 3)
...
/// 12. `pending_chord_event` (Mutex) - Chord debounce, brief access only
/// 13. `event_processors` (DashMap) - Per-device processors, shard-level locking
///
/// ## Key Principles
///
...
///   action_dispatcher.try_dispatch() — LOCK-FREE hot path (ADR-015;
///   unified the legacy single-device path into this one)
...
pub struct EngineManager {
    ...
    /// §D9 watch target. Distinct from `config_path`, which is the
    /// daemon's loaded **authority** (`live.toml` when present). The "Overwrite
    /// user.toml" drift action writes HERE, not to `config_path`.
    ...
    /// Where `active_profile.json` (daemon-owned durable profile
    ...
    // Engine components.
    // (legacy singleton `event_processor` removed — every device
    ...
    /// intended as stage-9 of the 8-stage matcher (hot-path
    ...
    /// (ADR-036 §8). Written by `dispatch_route_outputs`; shared
    ...
    /// ADR-015 D1). Do NOT push dispatch config here — virtual ports, OSC
    ...
    /// instance. Setting them here was a past bug. Full type-split to make
    ...
    /// `executor_vport_tx`) is the dispatch authority.
    ...
    /// Latest-wins desired virtual-port names → the executor thread,
    ...
    /// Coalesces `midi_*_suppressed` MonitorEvents so a
    ...
    /// declares `[[bindings]]`. (ADR-009 Phase 2; legacy
    /// single-device channel removed.)
    ///
    /// ADR-039: the element type is `ProtocolEvent`, not `InputEvent` —
    ...
    /// Set while a spawned hot-plug enumeration is in flight, so a burst
    ...
    /// Guards the one-time spawn of the daemon-lifetime timer-tick (D12)
    ...
    /// MIDI Learn mode active flag
    midi_learn_active: Arc<AtomicBool>,

    /// Event monitor active flag — zero cost when disabled
    event_monitor_active: Arc<AtomicBool>,

    /// Event monitor ring buffer — capacity 1000
    /// Uses std::sync::Mutex with try_lock() on hot path (avoid async in event loop)
    event_monitor_buffer: Arc<std::sync::Mutex<VecDeque<MonitorEvent>>>,
    event_monitor_max: usize,

    /// Broadcast channel for push-based event monitoring
    ...
    /// Monotonic event sequence counter. Stamped onto every
    ...
    /// MIDI Learn event buffer - stores events for GUI polling
    ///
    /// ARCHITECTURE NOTE:
    ...
    /// Pending chord event for MIDI Learn debouncing
    ...
    /// Current mode state — atomic swap via ArcSwap for lock-free reads (ADR-009 Phase 3)
    ...
    /// concurrent callers (CLI/MCP task vs. the app-detector task).
    /// Mode changes are infrequent, so holding it across the persist I/O is fine.
    mode_mutation_lock: tokio::sync::Mutex<()>,

    /// Rule set version counter for debugging/logging
    rule_set_version: std::sync::atomic::AtomicU64,

    /// Tool executor for LLM integration (ADR-007 Phase 2).
    /// ADR-045 D1: IPC-only write machinery — `llm-executor` builds.
    #[cfg(feature = "llm-executor")]
    tool_executor: Arc<ToolExecutor>,

    /// Audit logger (ADR-027 D13a). The same `Arc` is shared
    ...
    /// ADR-045 D1: SQLite audit only exists in `audit-db` builds.
    ...
    /// ADR-045 D5: the write-side audit seam — SQLite when
    ...
    /// ADR-042 Phase B-early bind gate. Decides whether a **non-loopback**
    ...
    /// Per-device event rate limiter (ADR-009 D9)
    ...
    /// GUI's current UI mode ("llm" | "studio") — ADR-032 P4.
    ...
    /// Channel to re-target ConfigWatcher after profile switch
    watcher_retarget_tx: Option<mpsc::Sender<PathBuf>>,

    /// Automatic profile switching by frontmost app
    app_detector: Option<Arc<crate::daemon::app_detector::DaemonAppDetector>>,

    /// Suppression timestamp for config watcher feedback loop.
    ...
    /// Profile cache for fast switching (R770: <50ms target).
    ...
    /// ADR-025 Phase 3.F runtime check. Handle for the deferred
    ...
    /// ADR-025 Phase 3.F cooperative cancellation flag.
    ...
    /// Content revision (ADR-043 D2/Q2) of the last `LiveConfig`
...
/// MIDI event for MIDI Learn feature
///
...
/// Uses typed enums instead of strings (ADR-004)
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MidiLearnEvent {
    /// Event type using type-safe enum (ADR-004)
    pub event_type: EventType,
    /// Source device identity (ADR-009 Phase 2)
    ...
    // Pattern detection fields (ADR-002)
    /// Detected pattern type using type-safe enum (ADR-004)
...
// `extract_raw_midi` was extracted to crate::midi_bytes so the small
// surface (one function + 9 tests) can be reviewed in full by tooling
// that has input-size limits — this file is too large for a
// balanced-tier 30k cap.
