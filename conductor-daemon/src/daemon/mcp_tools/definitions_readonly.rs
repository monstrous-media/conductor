...
pub(super) fn readonly_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            description: "Return the RUNTIME-RESOLVED routing graph (ADR-031 §3.4) from the daemon's `connector_registry` + `config.routes`. Distinct from `conductor_get_routing_graph` which returns the DECLARED (config) view: this tool reflects the live state — bindings lowered into input connectors, explicit `[[connectors]]` folded in, each route's `from`/`to` resolved against the registry so `from_missing`/`to_missing` surface validator-bypassed routes. Response: { connectors: [{ alias, direction, protocol, enabled, connected, bound_port, description, channels }], routes: [{ key, from_alias, to_alias, from_missing, to_missing, enabled, filter, transform, description }] }. The canonical source for routing-graph rendering — replaces GUI-side `projectRoutingGraph` / `bindingToConnector` re-implementations.".to_string(),
            input_schema: json!({
                "type": "object",
                "properties": {},
                "required": []
