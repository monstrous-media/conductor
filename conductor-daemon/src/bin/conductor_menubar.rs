...
mod app {
    use conductor_daemon::daemon::{
        IconState, IpcClient, IpcCommand, IpcRequest, MenuAction, MenuBar, MenuBarError,
        ResponseStatus,
    };
...
// tray-icon (and the menu_bar module) are macOS-only: on Linux the daemon
// runs headless and the tray stack would drag in the unmaintained GTK3
// binding tree for a surface nothing ships.
#[cfg(not(target_os = "macos"))]
fn main() {
    eprintln!(
        "conductor-menubar is the macOS system-tray helper; it is not available on this platform."
    );
    std::process::exit(2);
}
